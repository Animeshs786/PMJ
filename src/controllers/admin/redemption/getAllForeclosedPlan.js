...
exports.getAllForclosedPlans = catchAsync(async (req, res, next) => {
  const {
    collectionAgent,
    salePerson,
    userId,
  ...
  if ((storeIds && storeIds.length > 0) || (salePersonIds && salePersonIds.length > 0)) {
    ...
    if (storeIds && storeIds.length > 0) {
      ...
      if (storeUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No forclosed plans found for these stores.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage) || 1,
          results: 0,
    ...
    if (salePersonIds && salePersonIds.length > 0) {
      ...
      if (salePersonUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No forclosed plans found for these sale persons.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage) || 1,
          results: 0,
    ...
    if (userIds.length === 0) {
      return res.status(200).json({
        status: true,
        message: "No forclosed plans found matching the provided filters.",
        totalResult: 0,
        totalPage: 0,
        currentPage: parseInt(currentPage) || 1,
        results: 0,
  ...
  if (collectionAgent) {
    ...
    const collectionAgentUserIds = assignments.map((assignment) => assignment.user).filter(Boolean);
    ...
    if (filter.user) {
      filter.user.$in = filter.user.$in.filter(id => collectionAgentUserIds.includes(id));
  ...
  // Fetch forclosed plans
  const forclosedPlans = await UserPlan.find({
    ...filter,
  ...
  // Filter out plans where user or planDock is null and add additional fields
