...
// exports.getAllRedemption = catchAsync(async (req, res, next) => {
//   const {
//     collectionAgent,
//     salePerson,
//     userId,
...
//     const userIds = assignments.map((assignment) => assignment.user).filter(Boolean);
//     if (userIds.length === 0) {
...
//   const redeemedPlans = await UserPlan.find({
//     ...filter,
...
//         ...(Object.keys(collectionAgentMatch).length > 0 ? collectionAgentMatch : {}),
//       },
//     })
//     .populate({
//       path: "planDock",
...
//   // Filter out plans where user or planDock is null due to match
//   const filteredPlans = redeemedPlans.filter((plan) => plan.user && plan.planDock);
...
exports.getAllRedemption = catchAsync(async (req, res, next) => {
  ...
  if (planDock) {
    const planDockExists = await PlanDock.findById(planDock).populate(
      "plan",
      "name"
    );
    if (!planDockExists) {
  ...
  if (
    ...
    if (storeIds && storeIds.length > 0) {
      ...
      if (storeUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No redeemed plans found for these stores.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage) || 1,
          results: 0,
    ...
    if (salePersonIds && salePersonIds.length > 0) {
      ...
      if (salePersonUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No redeemed plans found for these sale persons.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage) || 1,
          results: 0,
    ...
    if (userIds.length === 0) {
      return res.status(200).json({
        status: true,
        message: "No redeemed plans found matching the provided filters.",
        totalResult: 0,
        totalPage: 0,
        currentPage: parseInt(currentPage) || 1,
        results: 0,
