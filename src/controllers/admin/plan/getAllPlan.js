...
exports.getAllPlans = catchAsync(async (req, res, next) => {
  ...
  if (dateFilter) {
    ...
    // If no date filter is provided, include all plans (no createdAt restriction)
    // Optionally, keep existing startDate/endDate logic for backward compatibility
    if (startDate) {
  ...
  // Search Filter Logic for name or commitedAmount
  if (search) {
    // Check if search is a number for commitedAmount
    const isNumber = !isNaN(search) && !isNaN(parseFloat(search));
    filter.$or = [
      { name: { $regex: search, $options: "i" } }, // Case-insensitive search for name
    ];
    if (isNumber) {
      filter.$or.push({ commitedAmount: parseFloat(search) }); // Exact match for commitedAmount
    }
  }
  ...
  // Fetch plans with filter, pagination, and sorting
  const plans = await Plan.find(filter)
    .sort("-createdAt")
    .skip(skip)
    .limit(limit);
