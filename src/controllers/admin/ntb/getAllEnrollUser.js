...
// exports.getEnrollUsersForAdmin = catchAsync(async (req, res, next) => {
//   const {
//     search,
//     dateFilter,
//     startDate,
...
//   // Fetch enrolled user plans
//   const enrolledUserPlans = await UserPlan.find(filter)
//     .populate({
//       path: "user",
...
//   const filteredPlans = enrolledUserPlans.filter((plan) => plan.user);
  ...
  if ((storeIds && storeIds.length > 0) || (salePersonIds && salePersonIds.length > 0)) {
    ...
    if (storeIds && storeIds.length > 0) {
      ...
      if (storeUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No enrolled user plans found for these stores.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage),
          results: 0,
    ...
    if (salePersonIds && salePersonIds.length > 0) {
      ...
      if (salePersonUserIds.length === 0) {
        return res.status(200).json({
          status: true,
          message: "No enrolled user plans found for these sale persons.",
          totalResult: 0,
          totalPage: 0,
          currentPage: parseInt(currentPage),
          results: 0,
    ...
    if (userIds.length === 0) {
      return res.status(200).json({
        status: true,
        message: "No enrolled user plans found matching the provided filters.",
        totalResult: 0,
        totalPage: 0,
        currentPage: parseInt(currentPage),
        results: 0,
      ...
      if (collectionAgentUserIds.length > 0) {
        if (filter.user) {
          ...
          if (filter.user.$in.length === 0) {
            return res.status(200).json({
              status: true,
              message: "No enrolled user plans found matching the search criteria.",
              totalResult: 0,
              totalPage: 0,
              currentPage: parseInt(currentPage),
              results: 0,
