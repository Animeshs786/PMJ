...
// exports.getOverdueUsersForAdmin = catchAsync(async (req, res, next) => {
//   const {
//     collectionAgent,
//     salePerson,
//     userId,
...
//   const paginatedOverdueUsers = overdueUsers.slice(skip, skip + limitNum);
...
exports.getOverdueUsersForAdmin = catchAsync(async (req, res, next) => {
  ...
  if (dateFilter) {
    ...
    // Default: Earliest to start of current month
    const now = new Date();
    const currentMonth = now.getMonth();
    const currentYear = now.getFullYear();
    start = new Date(0); // Earliest possible date
