...
//   const { collectionAgentId, salePersonId, page = 1, limit = 10 } = req.query;
//   const now = new Date();
...
//     const query = { user: user._id, status: "Active" };
//     if (salePersonId) {
//       query.salePersonId = salePersonId;
//     }

//     const activePlans = await UserPlan.find(query).populate("plan", "name commitedAmount");
...
//   const paginatedCollectedUsers = collectedUsers.slice(startIndex, startIndex + limit);

//   res.status(200).json({
//     status: true,
//     message: "Current month's collected users retrieved successfully.",
//     data: {
//       total: collectedUsers.length,
//       page: parseInt(page),
...
//   const remainingPendingPayments = emiList.emiList.filter((emi) => emi.status === "Pending").length;

//   let walletAmount = emiList.emiList.reduce(
//     (total, emi) => (emi.status === "Paid" ? total + plan.commitedAmount : total),
//     0
//   );
...
exports.getCurrentMonthCollectedUsers = catchAsync(async (req, res, next) => {
  const {
    collectionAgent,
    salePerson,
    userId,
  ...
  for (const plan of filteredPlans) {
    ...
    if (emiRecords && emiRecords.emiList.length > 0) {
      const currentMonthEMIs = emiRecords.emiList.filter((emi) => {
        const paidDate = emi.paidDate ? new Date(emi.paidDate) : null;
        return (
          emi.status === "Paid" &&
          paidDate &&
  ...
  const paginatedCollectedUsers = collectedUsers.slice(skip, skip + limitNum);
