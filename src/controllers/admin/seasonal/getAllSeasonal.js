...
//     filter
//   );
//   const exhibitions = await Seasonal.find(filter).skip(skip).limit(limit);
//   res.status(200).json({
//     status: true,
//     totalResult,
//     totalPage,
...
exports.getAllSeasonal = catchAsync(async (req, res, next) => {
  const {
    search,
    dateFilter,
    startDate,
  ...
  const seasonals = await Seasonal.find(filter)
    .skip(skip)
    .limit(limit)
    .sort("-createdAt");
