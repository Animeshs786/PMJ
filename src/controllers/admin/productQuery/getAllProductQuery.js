...
exports.getAllProductQueries = catchAsync(async (req, res, next) => {
  const {
    search,
    dateFilter,
    startDate,
  ...
  // Search Filter Logic for name and product's name
  if (search) {
    filter.$or = [{ name: { $regex: search, $options: "i" } }]; // Search query's name

    // Find product IDs where product.name matches the search term
  ...
  // Fetch queries with filter, pagination, and sorting
  const queries = await ProductQuery.find(filter)
    .sort("-createdAt")
    .skip(skip)
    .limit(limit)
    .populate("product", "name"); // Populate product name for clarity
