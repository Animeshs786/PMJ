...
// exports.getAllServiceQury = catchAsync(async (req, res, next) => {
...
//   console.log(salePersonId, "sldjfslj");
...
exports.getAllServiceQury = catchAsync(async (req, res, next) => {
  ...
  console.log("SalePerson ID:", salePersonId);
  ...
  // Fetch shared users for the salesperson
  ...
  if (!sharedUsers.length) {
    return res.status(200).json({
      ...
      message: "No shared users found for the salesperson.",
    });
  }

  // Convert mobile strings to numbers for matching Service.mobileNumber
  const sharedMobileNumbers = sharedUsers
    .map((share) => {
      const mobileNum = parseInt(share.mobile, 10);
      return isNaN(mobileNum) ? null : mobileNum;
    })
    .filter(num => num !== null);


  if (!sharedMobileNumbers.length) {
    return res.status(200).json({
      status: true,
      totalResult: 0,
      totalPage: 0,
      ...
      message: "No valid mobile numbers found for the shared users.",
    });
  }
