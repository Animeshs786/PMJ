...
// exports.getSinglePurchasePlans = catchAsync(async (req, res, next) => {
...
//   const plans = await UserPlan.find({ salePersonId })
...
exports.getSinglePurchasePlans = catchAsync(async (req, res, next) => {
  ...
  // Get pagination and date filter parameters from query
  const {
    page = 1,
    limit = 10,
  ...
  // Fetch all plans for the salesperson
  const allPlans = await UserPlan.find(filter)
    .populate("user", "name mobile")
    .sort("-createdAt");

  // Group plans by user and filter those with exactly one plan
  const userPlanCount = {};
  const userActivePlanStatus = {}; // Track users with active plans

  allPlans.forEach((plan) => {
    const userId = plan.user._id.toString();
    userPlanCount[userId] = (userPlanCount[userId] || 0) + 1;
  ...
  // Filter users with exactly one plan and no active plans
  const singlePlans = allPlans.filter((plan) => {
    const userId = plan.user._id.toString();
    return (
      userPlanCount[userId] === 1 && // User has exactly one plan
      !userActivePlanStatus[userId] // User does not have any active plans
    );
  });

  // Apply pagination to the filtered single plans
  const totalCount = singlePlans.length;
  const paginatedPlans = singlePlans.slice(
    (pageNum - 1) * limitNum,
    pageNum * limitNum
  );

  const response = paginatedPlans.map((plan) => ({
