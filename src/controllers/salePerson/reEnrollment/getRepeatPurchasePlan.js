...
// exports.getRepeatPurchasePlans = catchAsync(async (req, res, next) => {
...
//   const totalCount = repeatPlans.length;
//   const paginatedPlans = repeatPlans.slice(
//     (pageNum - 1) * limitNum,
//     pageNum * limitNum
//   );
...
//   // Fetch all plans for the salesperson with an "Active" status
//   const plans = await UserPlan.find({ salePersonId, status: "Active" })
//     .populate("user", "name mobile")
//     .sort("-createdAt");

//   // Count the number of plans for each user
//   const userPlanCount = {};
//   plans.forEach((plan) => {
//     userPlanCount[plan.user._id] = (userPlanCount[plan.user._id] || 0) + 1;
//   });

//   // Filter users who have more than one "Active" plan
//   const repeatPlans = plans.filter((plan) => userPlanCount[plan.user._id] > 1);
...
exports.getRepeatPurchasePlans = catchAsync(async (req, res, next) => {
  ...
  const filter = { salePersonId, status: "Active" };
