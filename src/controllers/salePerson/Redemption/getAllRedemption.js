...
//     return next(new AppError("No active user found for the sale person", 404));
//   }
...
//     filter.redemptionDate = { $gte: start, $lte: end }; // Use redemptionDate for filtering
//   }
...
exports.getAllRedemption = catchAsync(async (req, res, next) => {
  ...
  if (dateFilter) {
    ...
    filter.redemptionDate = { $gte: start, $lte: end };
  }

  // Find user plans with 11 paid EMIs
  ...
  if (eligibleUserPlanIds.length === 0) {
    return next(new AppError("No eligible redemptions found.", 404));
  }
